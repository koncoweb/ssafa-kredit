...
export default function CustomersManagement() {
  ...
  const handlePrintPaymentsHistory = async () => {
    ...
    try {
      const items = filteredPayments.map((tx) => ({
        id: tx.id,
        ...
        renderItem={({ item: cust }) => (
          <Card key={cust.uid} style={{ marginBottom: 12 }} mode="elevated">
            ...
            <Card.Actions>
              ...
              <Button mode="contained-tonal" onPress={() => router.push({ pathname: '/(admin)/reports', params: { customerId: cust.uid, customerName: cust.name } })}>
                Riwayat Transaksi
              </Button>
            </Card.Actions>
          </Card>
                          ...
                          <Card.Content>
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                              ...
                              <IconButton
                                ...
                                disabled={!canPrint || printingPaymentId === tx.id}
